...
@Autonomous (name = "autonWobbleGoalARing", group = "primary")
public class autonWobbleGoalARing extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        pid = new PIDController3(0.0007, 0.0000035, 0.00007, 20);
        waitForStart();
        while (opModeIsActive()) {
            ...
            //prbot.getDrawbridge().getPulleyBoi().setPower(0.01);
            //sleep(200);
            ...
            prbot.getDrivetrain().stop();
            sleep(500);
            // 60 at field. 54 at home
            prbot.getDrivetrain().forward(60);
            sleep(1000);
            setpoint = Constants.SHOOT_UP_SETPOINT;
...
//                    setpoint += 2;
//                }
                if (elapsedTime.milliseconds() > 22000) {
                    prbot.getDrawbridge().getPulleyBoi().setPower(0);
                    break;
                }
...
//            }
            while (elapsedTime.milliseconds() < 12000) {
                prbot.getDrawbridge().getPulleyBoi().setPower(pid.output(setpoint, prbot.getDrawbridge().getPulleyBoi().getCurrentPosition()));
            }
            prbot.getOuttake().shoot();
            while (elapsedTime.milliseconds() < 17000) {
                prbot.getDrawbridge().getPulleyBoi().setPower(pid.output(setpoint, prbot.getDrawbridge().getPulleyBoi().getCurrentPosition()));
            }
            prbot.getOuttake().stop();
            while (elapsedTime.milliseconds() < 22000) {
                if (setpoint > 0) {
                    setpoint--;
                }
                telemetry.addData("setpoint", setpoint);
